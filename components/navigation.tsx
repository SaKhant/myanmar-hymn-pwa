...
export function Navigation() {
  ...
  return <>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-[var(--line)] bg-[var(--paper)] pb-[env(safe-area-inset-bottom)] md:hidden" aria-label="Main navigation">{items.map(({id,href,label,icon:Icon}) => { const active=isActive(href); return <Link key={href} href={href} data-tab={id} aria-current={active?"page":undefined} className="bottom-nav-link focus-ring flex min-h-16 flex-col items-center justify-center gap-1 text-[10px] font-semibold">{active&&id==="hymns"?<OpenBookIcon/>:active&&id==="yp"?<SongLyricsIcon/>:active&&id==="favorites"?<LikeIcon/>:active&&id==="settings"?<FilledSettings/>:<Icon size={21}/>}<span>{label}</span></Link>; })}</nav>
  </>;
}
