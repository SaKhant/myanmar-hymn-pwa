...
function OfflineRoute({hymns,categories,pathname}:{hymns:OfflineHymn[];categories:HymnCategory[];pathname:string}){
  ...
  if(parts[0]==="settings")return <main className="page max-w-3xl"><h1 className="font-serif text-4xl md:text-5xl">Settings</h1><p className="mt-3 text-[var(--muted)]">Preferences stay on this device. No account is needed.</p><section className="surface mt-8 p-6"><h2 className="myanmar text-xl font-bold">ဖုန်းတွင် App အဖြစ် ထည့်သွင်းရန်</h2><div className="mt-5 space-y-5 text-sm leading-7 text-[var(--muted)]"><div><h3 className="font-bold text-[var(--ink)]">Android</h3><p className="myanmar mt-1">Chrome ဖြင့် ဝဘ်ဆိုဒ်ကိုဖွင့်ပါ → ⋮ ကိုနှိပ်ပါ → Install app သို့မဟုတ် Add to Home screen ကိုရွေးပါ။</p></div><div><h3 className="font-bold text-[var(--ink)]">iPhone / iPad</h3><p className="myanmar mt-1">Safari ဖြင့် ဝဘ်ဆိုဒ်ကိုဖွင့်ပါ → Share ⬆️ ကိုနှိပ်ပါ → Add to Home Screen → Add ကိုနှိပ်ပါ။</p></div><p className="myanmar">ထည့်သွင်းပြီးနောက် ပုံမှန် App တစ်ခုကဲ့သို့ Home Screen မှ တိုက်ရိုက်ဖွင့်နိုင်ပါသည်။</p></div></section><p className="mt-6 text-sm text-[var(--muted)]">Audio need internet or wifi.</p></main>;
