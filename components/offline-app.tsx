  ...
  return <main className="page reader-page">
    ...
    <article><header className="border-b border-[var(--line)] pb-5">
      <p className="eyebrow reader-kicker">
        ...
        {kind==="hymns"&&isMyanmar&&reference&&<><span className="reader-version-separator">•</span>{englishHymn?<Link href={`/hymns/en/${englishHymn.id}`} className="reader-version-link focus-ring">ENG {reference}</Link>:<span className="reader-version-reference">ENG {reference}</span>}</>}
        {kind==="hymns"&&!isMyanmar&&relatedMyanmar&&<><Link href={`/hymns/my/${relatedMyanmar.id}`} className="reader-version-link focus-ring">MY {relatedMyanmar.number??relatedMyanmar.id}</Link><span className="reader-version-separator">•</span></>}
        {kind==="hymns"&&!isMyanmar&&<Link href={`/hymns/en/${hymn.id}`} aria-current="page" className="reader-current-version reader-version-link focus-ring">ENG {relatedMyanmar?.cross_references.Eng?.trim()||hymn.number||hymn.id}</Link>}
...
function OfflineRoute({hymns,categories}:{hymns:OfflineHymn[];categories:HymnCategory[]}){
  ...
  if(parts[0]==="settings")return <main className="page max-w-3xl"><h1 className="font-serif text-4xl md:text-5xl">Settings</h1><p className="mt-3 text-[var(--muted)]">Preferences stay on this device. No account is needed.</p><OfflineLibrarySettings/><section className="mt-8"><h2 className="font-serif text-2xl">Offline use</h2><p className="mt-2 max-w-xl text-sm leading-6 text-[var(--muted)]">Install Hymn App from your browser’s menu for quick Home Screen access. Download the Offline Library above to keep hymn and song text available without internet; audio still requires a connection.</p></section></main>;
