...
function HymnList({results,kind,language,exactNumber}:{results:HymnSummary[];kind:HymnKind;language:HymnLanguage;exactNumber?:string}) {
  ...
  if(compactMyanmarHymns){
    return <div className="grid gap-0.5">{results.map(h=>{const exact=String(h.number)===exactNumber;return <Link key={h.id} href={`/${kind}/${language}/${h.id}`} className={`focus-ring grid min-h-0 grid-cols-[52px_minmax(0,1fr)] items-center gap-3 rounded-[10px] px-2 py-3 text-black no-underline hover:bg-[var(--sage-soft)] ${exact?"bg-[var(--sage-soft)]":""}`}><span className="tabular-nums text-[0.88rem] text-black">{displayNumber(h.number)}</span><span className="myanmar min-w-0 text-base font-normal leading-[1.4] text-black">{h.title}</span></Link>})}</div>;
  }
