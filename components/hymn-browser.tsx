...
function SearchBarWithMenu({kind,query,onQueryChange,onSubmit,menuOpen,onMenuToggle,onMenuClose}:{kind:HymnKind;query:string;onQueryChange:(value:string)=>void;onSubmit:()=>void;menuOpen:boolean;onMenuToggle:()=>void;onMenuClose:()=>void}){
  ...
  return <div className="w-full max-w-md rounded-xl bg-blue-100 p-1.5"><div className="flex items-center gap-2"><BlueSearchForm query={query} onQueryChange={onQueryChange} onSubmit={onSubmit}/><div className="relative shrink-0"><button type="button" aria-label="Open menu" title="Menu" aria-expanded={menuOpen} aria-controls={menuId} onClick={onMenuToggle} className="focus-ring flex h-11 w-11 items-center justify-center rounded-lg bg-blue-500 text-white transition hover:bg-blue-600"><Menu aria-hidden="true" size={27} strokeWidth={2.75}/></button>{menuOpen&&<div id={menuId} role="menu" className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-44 min-w-44 max-w-44 overflow-hidden rounded-xl border border-blue-600 bg-blue-500 p-1.5 shadow-xl shadow-blue-300/50"><Link href="/categories" role="menuitem" onClick={onMenuClose} style={{color:"#fff"}} className={menuItemClass}><List aria-hidden="true" size={17} color="#fff"/><span style={{color:"#fff"}} className="truncate">မာတိကာ ဇယား</span></Link><Link href={newTranslationsHref} role="menuitem" onClick={onMenuClose} style={{color:"#fff"}} className={menuItemClass}><FileText aria-hidden="true" size={17} color="#fff"/><span style={{color:"#fff"}} className="truncate">ဘာသာပြန် အသစ်</span></Link></div>}</div></div></div>;
}
