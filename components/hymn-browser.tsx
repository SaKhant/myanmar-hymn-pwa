...
import { BookOpen, ChevronRight, FileText, List, Menu, Music2, Search } from "lucide-react";
...
export function HymnBrowser({ kind, myanmar, english=[], newTranslations=[], initialLanguage="my", initialQuery="", myanmarOnly=false, lyricSearchUrl }:{ kind:HymnKind; myanmar:HymnSummary[]; english?:HymnSummary[]; newTranslations?:NewTranslationSummary[]; initialLanguage?:HymnLanguage; initialQuery?:string; myanmarOnly?:boolean; lyricSearchUrl?:string }) {
  ...
  const [newTranslationsMenuOpen,setNewTranslationsMenuOpen]=useState(false);
  ...
  return <main className="page">
    ...
    {kind==="hymns"?<div className="w-full max-w-md rounded-xl bg-blue-100 p-1.5"><div className="flex items-center gap-2"><BlueSearchForm query={query} onQueryChange={updateQuery} onSubmit={submitSearch}/><div className="relative shrink-0"><button type="button" aria-label="Open menu" title="Menu" aria-expanded={menuOpen} aria-controls="hymns-quick-menu" onClick={()=>{setMenuOpen(open=>!open);if(menuOpen)setNewTranslationsMenuOpen(false)}} className="focus-ring flex h-11 w-11 items-center justify-center rounded-lg border border-black bg-blue-400 text-white transition hover:bg-blue-500"><Menu aria-hidden="true" size={22}/></button>{menuOpen&&<div id="hymns-quick-menu" role="menu" className="absolute right-0 top-[calc(100%+0.5rem)] z-50 w-48 rounded-xl border border-blue-600 bg-blue-500 p-1.5 shadow-xl shadow-blue-300/50"><Link href="/categories" role="menuitem" onClick={()=>setMenuOpen(false)} style={{color:"#fff"}} className="focus-ring flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-semibold no-underline transition hover:bg-blue-600"><List aria-hidden="true" size={17} color="#fff"/><span style={{color:"#fff"}}>Categories</span></Link><button type="button" role="menuitem" aria-expanded={newTranslationsMenuOpen} onClick={()=>setNewTranslationsMenuOpen(open=>!open)} style={{color:"#fff"}} className="focus-ring flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-semibold transition hover:bg-blue-600"><FileText aria-hidden="true" size={17} color="#fff"/><span style={{color:"#fff"}}>New translations</span><ChevronRight aria-hidden="true" size={16} color="#fff" className={`ml-auto transition-transform ${newTranslationsMenuOpen?"rotate-90":""}`}/></button>{newTranslationsMenuOpen&&<div className="ml-4 border-l border-white/35 pl-2"><Link href="/hymns/new-translations" role="menuitem" onClick={()=>{setMenuOpen(false);setNewTranslationsMenuOpen(false)}} style={{color:"#fff"}} className="focus-ring flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold no-underline transition hover:bg-blue-600"><BookOpen aria-hidden="true" size={15} color="#fff"/><span style={{color:"#fff"}}>Hymns</span></Link><Link href="/hymns/new-translations?section=yp" role="menuitem" onClick={()=>{setMenuOpen(false);setNewTranslationsMenuOpen(false)}} style={{color:"#fff"}} className="focus-ring flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold no-underline transition hover:bg-blue-600"><Music2 aria-hidden="true" size={15} color="#fff"/><span style={{color:"#fff"}}>YP Songs</span></Link></div>}</div>}</div></div></div>:<div className="w-full max-w-md rounded-xl bg-blue-100 p-1.5"><div className="flex items-center gap-2"><BlueSearchForm query={query} onQueryChange={updateQuery} onSubmit={submitSearch}/></div></div>}
