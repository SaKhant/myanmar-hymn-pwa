...
export function YpGuitarReader({sections,ypNumber,sourceLabel}:{sections:HymnSection[];ypNumber:number;sourceLabel:string}){
  ...
  async function showGuitar(){
    ...
    setError(null);
    try {
      const response=await fetch(`/api/yp-guitar/${ypNumber}`);
...
function UnavailableGuitar({message}:{message:string|null}){
  return <div className="mx-auto max-w-2xl py-10 text-center text-sm font-semibold text-[var(--muted)]">{message??"Structured guitar chords are not available for this song."}</div>;
}
