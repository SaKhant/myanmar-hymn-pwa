...
export default function CategoriesPage(){const categories=[...getCategories()].sort((a,b)=>categoryNumber(a.category)-categoryNumber(b.category)||a.category.localeCompare(b.category));return <main className="page"><ReaderBackButton fallback="/" label="Back to Hymns"/><p className="eyebrow">Myanmar hymn book</p><h1 className="mt-2 font-serif text-4xl md:text-5xl">Categories</h1><p className="mt-3 text-[var(--muted)]">Browse 33 themes from the source collection.</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{categories.map(c=>{const references=[...c.hymns].sort((a,b)=>a.number-b.number);return <section key={c.slug} className="surface p-5"><h2 className="myanmar font-bold">{categoryTitle(c)}</h2><p className="mt-2 text-xs text-[var(--muted)]">{references.length} hymn references</p><div className="mt-4 flex flex-wrap gap-2">{references.map(h=><Link key={h.number} href={`/hymns/my/${h.number}`} className="focus-ring rounded-full bg-[var(--sage-soft)] px-3 py-1.5 text-xs font-bold text-[var(--sage)]">{h.number}</Link>)}</div></section>})}</div></main>}
