...
export default function CategoriesPage(){const categories=[...getCategories()].sort((a,b)=>categoryNumber(a.category)-categoryNumber(b.category)||a.category.localeCompare(b.category));return <main className="page"><p className="eyebrow">Myanmar hymn book</p><h1 className="mt-2 font-serif text-4xl md:text-5xl">Categories</h1><p className="mt-3 text-[var(--muted)]">Browse 33 themes from the source collection.</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{categories.map(c=><section key={c.slug} className="surface p-5"><h2 className="myanmar font-bold">{categoryTitle(c)}</h2></section>)}</div></main>}
